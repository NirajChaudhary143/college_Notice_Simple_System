...
<body style="background-image: linear-gradient(to right, #ccc,#eee,#eee,#ccc);">
                ...
                <div class="d-flex justify-content-center form_container">
                    <form class="formm" method="POST" action="login.php">
                        ...
                        <div class="extra">
                            ...
                            <a href="register.php">Create New Accout</a>
                        </div>
                    </form>
                </div>
            </div>
    ...
    if (isset($_POST['signin'])) {
        ...
        $sql = "SELECT * FROM user_login WHERE user_name = '$user_name' AND user_password = '$user_password'";
        $result = $conn->query($sql);

        if($result->num_rows==1){
            session_start();
            $_SESSION['AdminLoginId']=$user_name;
            header("location: index.php");
        } else {
